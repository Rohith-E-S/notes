...
if (isset($_GET['id'])) {
    ...
    if ($task_result->num_rows > 0) {
        ...
        header("Location: ../notes/display.php?error=Task not found");
        exit();
    }
} else {
    header("Location: ../notes/display.php?error=Invalid task ID");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($update_stmt->execute()) {
        header("Location: ../notes/display.php?message=Task updated successfully");
        exit();
    } else {

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password !== $confirm_password) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: login.php?success=Password has been reset successfully. Please login with your new password.");
            exit();
        } else {
...
<body class="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-200">
    <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg w-96 transition-all duration-300 transform hover:scale-105">
        ...
        <form action="" method="POST" class="space-y-4">
            <div>
                <label class="block mb-2 text-gray-700 dark:text-gray-300">New Password:</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <i class="fas fa-lock text-gray-400"></i>
                    </div>
                    <input type="password" name="new_password" class="w-full p-2.5 pl-10 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                </div>
            </div>
            
            <div>
                <label class="block mb-2 text-gray-700 dark:text-gray-300">Confirm New Password:</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <i class="fas fa-lock text-gray-400"></i>
                    </div>
                    <input type="password" name="confirm_password" class="w-full p-2.5 pl-10 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                </div>
            </div>
